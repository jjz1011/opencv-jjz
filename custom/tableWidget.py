...
class TableWidget(QTableWidget):
    def __init__(self, parent=None):
        ...
        self.horizontalHeader().setStretchLastSection(True) #最后一列对窗口长度动态变化
        self.setFocusPolicy(Qt.NoFocus) #设置焦点事件

    def signal_connect(self):
        for spinbox in self.findChildren(QSpinBox):
            spinbox.valueChanged.connect(self.update_item) #整数和离散值
        for doublespinbox in self.findChildren(QDoubleSpinBox):
            doublespinbox.valueChanged.connect(self.update_item) #浮点数
        for combox in self.findChildren(QComboBox):
            combox.currentIndexChanged.connect(self.update_item) #下拉框
        for checkbox in self.findChildren(QCheckBox):
            checkbox.stateChanged.connect(self.update_item) #复选框
